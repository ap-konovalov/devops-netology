...
текстовый файл на удалённом хосте по пути, определённом в параметре `path`, с содержимым, определённым в параметре `content`.
4. Проверьте module на исполняемость локально.
`ansible -m my_own_module.py -a "path=example.txt content='lets learn netology ansible'" localhost`
...
- name: Create file
  ...
  tasks:
  - name: Create .txt file with content
    my_own_module:
      path: "example.txt"
      content: "hello world"
```
